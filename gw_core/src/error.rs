...
impl From<sqlx::Error> for DatabaseError {
    fn from(value: sqlx::Error) -> Self {
        match value {
            ...
            sqlx::Error::ColumnIndexOutOfBounds { index, len } => DatabaseError::QueryError(format!("index={index}, len={len} out of bounds")),
            sqlx::Error::ColumnNotFound(error) => DatabaseError::QueryError(format!("Column not found: {error}")),
            sqlx::Error::ColumnDecode { index: _, source } => DatabaseError::QueryError(source.to_string()),
